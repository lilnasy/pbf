import  { readFloatLE
	, readDoubleLE
	, writeFloatLE
	, writeDoubleLE} from './float/mod.ts'
...
class ProtoBuf {
	...
	readFields = <T>(readField: (tag: number, result: T, pbf: ProtoBuf) => void, result: T, end = this.length) => {
	...
	readMessage = <T>(readField: (tag: number, result: T, pbf: ProtoBuf) => void, result: T) => {
		return this.readFields(readField, result, this.readVarint(unsigned) + this.pos)
	}
	...
	readString = () => {
		...
		if (end - pos >= TEXT_DECODER_MIN_LENGTH && utf8TextDecoder) return readUtf8TextDecoder(this.buf, pos, end)
	...
	readPackedVarint = (arr: Array<number>, isSigned: boolean) => {
		if (this.type !== ProtoBuf.Bytes) return arr.push(this.readVarint(isSigned))
	...
	writeMessage = <T> (tag: number, fn: (obj: T, pbf: ProtoBuf) => void, obj: T) => {
...
function readPackedEnd(pbf: ProtoBuf) {
	...
	pbf.readVarint(unsigned) + pbf.pos : pbf.pos + 1
}
